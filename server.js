...
app.post("/report", async (req, res) => {
  ...
  for (let proxy of proxies) {
    ...
    try {
      ...
      await driver.wait(
        until.elementLocated(By.xpath("//iframe[contains(@src, 'recaptcha')]")),
        20000
      );
      const captchaIframe = await driver.findElement(
      ...
      if (taskResponse.data.status !== 1) {
        throw new Error(
          "Failed to create captcha task: " + taskResponse.data.request
        );
      }
      ...
      await driver.wait(
        until.elementLocated(By.id("g-recaptcha-response")),
        10000
      );
      const captchaResponseElement = await driver.findElement(
      ...
      for (let i = 0; i < phishLinks.length; i++) {
        ...
        try {
          ...
          await driver.wait(
            until.elementIsVisible(driver.findElement(By.id("url"))),
            15000
          );
          ...
          reportLines.push(
            `[PROXY: ${proxyAddress}:${port}] ${phishLink} ( Report Berhasil! )`
          );
          ...
          console.error(
            `[ERROR] An error occurred while processing link ${phishLink}:`,
            linkError
          );
  ...
  const reportFilePath = path.join(
    reportsDir,
    `report_phising_${reportFileDate}.txt`
  );
  ...
  res.send(
    `Telah berhasil melakukan bulking reports! Lihat laporan disini <a href="/reports/report_phising_${reportFileDate}.txt" download>Download Report</a>`
  ); // Success message
});
